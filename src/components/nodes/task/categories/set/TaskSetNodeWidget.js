...
import {taskSetNodeDefaultColor, TaskSetNodeModel} from './TaskSetNodeModel';

export class TaskSetNodeWidget extends React.Component {
    static defaultProps = {
        node: null,
        color: taskSetNodeDefaultColor
    };
    ...
    getInPorts() {
        ...
        if (displayOnly) {
            taskNode = new TaskSetNodeModel(node.name);
        }
    ...
    getOutPorts() {
        ...
        if (displayOnly) {
            taskNode = new TaskSetNodeModel(node.name, displayOnly);
        }
...
export const TaskSetNodeWidgetFactory = React.createFactory(TaskSetNodeWidget);
